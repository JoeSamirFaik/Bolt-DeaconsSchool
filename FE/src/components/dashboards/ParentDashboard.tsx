...
import { usersAPI, progressAPI, attendanceAPI } from '../../services/api';
...
const ParentDashboard: React.FC = () => {
  ...
  const { data: children, loading: childrenLoading } = useApi(() => usersAPI.getChildren());

  const stats = [
    { 
      label: 'عدد الأطفال', 
      value: childrenLoading ? '...' : `${children?.length || 0}`, 
      icon: ChartBarIcon, 
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 text-gray-800">
            <h2 className="text-2xl font-bold mb-2 font-cairo">جاري تحميل بيانات الأطفال...</h2>
            <p className="text-gray-600 font-cairo">نحضر معلومات أطفالك! 👨‍👩‍👧</p>
          </div>
        </div>
      </div>
    );
