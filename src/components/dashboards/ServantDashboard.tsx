...
const ServantDashboard: React.FC = () => {
  ...
  const { data: deacons, loading: deaconsLoading } = useApi(() => usersAPI.getDeacons());

  const stats = [
    { 
      label: 'الشمامسة', 
      value: deaconsLoading ? '...' : `${deacons?.length || 0}`, 
      icon: UsersIcon, 
  ...
  // Map real deacons data
  const recentAttendance = deacons?.slice(0, 4).map((deacon: any) => ({
    name: `${deacon.firstName} ${deacon.lastName}`,
    stage: deacon.stage || 'غير محدد',
    level: deacon.level || '1',
    status: deacon.isActive ? 'present' : 'absent',
    lastSeen: 'اليوم',
    avatar: '👦',
    mood: 'happy'
  })) || [];
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 text-gray-800">
            <h2 className="text-2xl font-bold mb-2 font-cairo">جاري تحميل بيانات الطلاب...</h2>
            <p className="text-gray-600 font-cairo">نحضر معلومات الشمامسة! 📚</p>
          </div>
        </div>
      </div>
    );
