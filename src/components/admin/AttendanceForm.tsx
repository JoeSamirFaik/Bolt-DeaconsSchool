      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <h1 className="text-lg sm:text-2xl font-bold text-gray-900 font-cairo">تسجيل الحضور</h1>
            <p className="text-gray-600 font-cairo text-sm sm:text-base">{session.name}</p>
          </div>
          
          {/* Left side - Back Button */}
          <button
            onClick={onClose}
            className="bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 sm:px-6 sm:py-3 rounded-xl transition-all duration-200 flex items-center space-x-2 space-x-reverse font-medium text-sm sm:text-base"
          >
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {error && (
          <div className="bg-white rounded-2xl shadow-sm border border-red-200 p-4">
            <p className="text-red-600 text-xs sm:text-sm font-medium text-right font-cairo">{error}</p>
          </div>
        )}

        {/* Session Info & Date Selection */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4 text-right">
            <div className="text-right">
              <h3 className="text-xs sm:text-sm font-medium text-gray-700 font-cairo">الجلسة</h3>
              <p className="text-base sm:text-lg font-semibold text-gray-900 font-cairo">{session.name}</p>
            </div>
            <div className="text-right">
              <h3 className="text-xs sm:text-sm font-medium text-gray-700 font-cairo">الوقت</h3>
              <p className="text-base sm:text-lg font-semibold text-gray-900 font-cairo">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 sm:px-4 sm:py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-right font-cairo text-sm sm:text-base"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 sm:gap-4">
            <div className="bg-white rounded-lg p-4 text-center">
              <div className="text-lg sm:text-2xl font-bold text-gray-900">{attendanceStats.total}</div>
              <div className="text-xs sm:text-sm text-gray-600 font-cairo">المجموع</div>
            </div>
            <div className="bg-white rounded-lg p-4 text-center">
              <div className="text-lg sm:text-2xl font-bold text-green-600">{attendanceStats.present}</div>
              <div className="text-xs sm:text-sm text-gray-600 font-cairo">حاضر</div>
            </div>
            <div className="bg-white rounded-lg p-4 text-center">
              <div className="text-lg sm:text-2xl font-bold text-yellow-600">{attendanceStats.late}</div>
              <div className="text-xs sm:text-sm text-gray-600 font-cairo">متأخر</div>
            </div>
            <div className="bg-white rounded-lg p-4 text-center">
              <div className="text-lg sm:text-2xl font-bold text-blue-600">{attendanceStats.excused}</div>
              <div className="text-xs sm:text-sm text-gray-600 font-cairo">معذور</div>
            </div>
            <div className="bg-white rounded-lg p-4 text-center">
              <div className="text-lg sm:text-2xl font-bold text-red-600">{attendanceStats.absent}</div>
              <div className="text-xs sm:text-sm text-gray-600 font-cairo">غائب</div>
            </div>
          </div>

          <div className="mt-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-base sm:text-lg font-bold text-green-600">{attendanceRate}%</span>
              <span className="text-xs sm:text-sm text-gray-600 font-cairo">نسبة الحضور</span>
            </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-3 sm:space-x-reverse">
              ...
              <input
                ...
                className="w-full sm:w-auto px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 text-right font-cairo text-sm"
              />
            </div>
            ...
            <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 sm:space-x-reverse w-full sm:w-auto">
              <button
                ...
                className="w-full sm:w-auto px-3 py-2 bg-green-100 text-green-800 rounded-lg hover:bg-green-200 transition-colors text-xs sm:text-sm font-medium font-cairo"
              >
                <span className="hidden sm:inline">تحديد الكل حاضر</span>
                <span className="sm:hidden">الكل حاضر</span>
              </button>
              <button
                type="button"
                onClick={() => markAllAs('absent')}
                className="w-full sm:w-auto px-3 py-2 bg-red-100 text-red-800 rounded-lg hover:bg-red-200 transition-colors text-xs sm:text-sm font-medium font-cairo"
              >
                <span className="hidden sm:inline">تحديد الكل غائب</span>
                <span className="sm:hidden">الكل غائب</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <div className="space-y-3">
            {filteredDeacons.map((deacon) => {
              ...
              return (
                <div
                  key={deacon.id}
                  className={`border-2 rounded-xl p-3 sm:p-4 transition-all ${getStatusColor(record.status)}`}
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-3 sm:space-y-0">
                    {/* Right side - Deacon Info */}
                    <div className="flex items-center space-x-3 sm:space-x-4 space-x-reverse">
                      <div className="text-right">
                        <h4 className="text-sm sm:text-base font-semibold text-gray-900 font-cairo">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 sm:space-x-reverse">
                      ...
                      <input
                        ...
                        placeholder="ملاحظات..."
                        className="w-full sm:w-32 px-2 py-1 sm:px-3 sm:py-1 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 text-xs sm:text-sm font-cairo"
                      />
                      ...
                      {(record.status === 'present' || record.status === 'late') && (
                        <input
                          ...
                          className="w-full sm:w-auto px-2 py-1 sm:px-3 sm:py-1 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 text-xs sm:text-sm font-cairo"
                          placeholder="وقت الوصول"
                        />
                      )}

                      {/* Status Buttons */}
                      <div className="flex space-x-1 space-x-reverse justify-center sm:justify-start">
                        <button
                          type="button"
          ...
          {filteredDeacons.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 font-cairo text-sm sm:text-base">لا يوجد شمامسة مؤهلون لهذه الجلسة</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 sm:space-x-reverse">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full sm:flex-1 px-4 py-3 bg-green-600 text-white rounded-xl hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium text-sm sm:text-base"
            >
              {loading ? (
